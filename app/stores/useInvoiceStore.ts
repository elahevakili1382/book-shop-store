...
const API_BASE = 'https://69215dda512fb4140be003df.mockapi.io'
const ENDPOINT = `${API_BASE}/invoices`
...
export const useInvoiceStore = defineStore('invoice', {
  ...
  actions: {
    async fetchInvoices() {
      ...
      this.error = null
      try {
        const data = await $fetch<Invoice[]>(ENDPOINT)
    ...
    async updateInvoice(id: string, payload: Partial<Invoice>) {
      try {
        const updated = await $fetch<Invoice>(`${ENDPOINT}/${id}`, {
          method: 'PUT',
          body: payload
        })
