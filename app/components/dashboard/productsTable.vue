  ...
  <div class="bg-dash-card border border-dash-border rounded-2xl p-6">
    <div class="flex items-center justify-between mb-5">
      ...
      <button class="px-4 py-2 rounded-xl bg-dash-accent text-dash-bg text-sm font-bold hover:opacity-90 transition"
        @click="openAddModal">+ افزودن محصول</button>
    </div>
          ...
          <tr v-for="product in paginatedProducts" :key="product.id"
            ...
            <td>
              <input type="number" min="0"
                ...
                v-model.number="product.quantity" @change="updateQuantity(product)" />
            </td>



            <td>
              <span class="px-3 py-1 rounded-full text-xs font-semibold " :class="product.quantity > 10
            ...
            <td class="flex gap-2 md:gap-3">
              ...
              <button class="text-rose-300 hover:text-rose-200" @click="deleteProduct(product)"> 🗑️ </button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-if="showEditModal" class="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div class="bg-dash-card border border-dash-border rounded-2xl w-full max-w-md p-6 text-dash-text">
        ...
        <form v-if="editProduct" @submit.prevent="submitEditProduct" class="space-y-4">
          <input v-model="editProduct!.title" type="text" placeholder="نام محصول"
            class="w-full border border-dash-border rounded-lg px-3 py-2 bg-dash-bg text-dash-text" required />
          <input v-model="editProduct!.price" type="number" placeholder="قیمت"
            class="w-full border border-dash-border rounded-lg px-3 py-2 bg-dash-bg text-dash-text" required />
          <select v-model="editProduct!.category"
            class="w-full border border-dash-border rounded-lg px-3 py-2 bg-dash-bg text-dash-text" required>
            <option v-for="cat in categories" :key="cat" :value="cat">{{ cat }}</option>
          </select>
          <input type="number" min="0" v-model.number="editProduct!.quantity"
            class="w-full border border-dash-border rounded-lg px-3 py-2 bg-dash-bg text-dash-text"
            placeholder="تعداد" />

          <div class="flex justify-end gap-3 pt-4">
            <button type="button" @click="closeEditModal"
              class="px-4 py-2 rounded-lg bg-dash-border text-dash-text">انصراف</button>
            <button type="submit"
              class="px-4 py-2 rounded-lg bg-dash-accent text-dash-bg font-bold hover:opacity-90">ذخیره</button>
          </div>
        </form>
    ...
    <div v-if="showAddModal" class="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div class="bg-dash-card border border-dash-border rounded-2xl w-full max-w-md p-6 text-dash-text">
        ...
        <form @submit.prevent="submitProduct" class="space-y-4">
          <input v-model="form.title" type="text" placeholder="نام محصول"
            class="w-full border border-dash-border rounded-lg px-3 py-2 bg-dash-bg text-dash-text transition" required>
          <input v-model="form.price" type="number" placeholder="قیمت"
            class="w-full border border-dash-border rounded-lg px-3 py-2 bg-dash-bg text-dash-text transition" required>
          ...
          <input type="number" min="0"
            ...
            placeholder="تعداد" v-model.number="form.quantity" />

          <div class="flex justify-end gap-3 pt-4">
            <button type="button" class="px-4 py-2 rounded-lg bg-dash-border text-dash-text"
              @click="closeAddModal">انصراف</button>
            <button type="submit"
              class="px-4 py-2 rounded-lg bg-dash-accent text-dash-bg font-bold hover:opacity-90">ذخیره</button>
          </div>
